...
const getInitialStockAlerts = () => {
  ...
  const now = new Date();
  const seed = [
    // Citizen Requests (autoGenerated: false)
    {
    ...
    // Hospital Requests (autoGenerated: true)
    {
...
const ChatWindow = ({ user, chatId, otherName, onClose }) => {
  ...
  useEffect(() => {
    const loadMessages = () => {
      ...
      // Okunmamış mesaj bildirimlerini okundu işaretle
...
const KanTalepleri = ({ user }) => {
  ...
  const handleApply = (alert) => {
    ...
    // Check if donation wait period is active
    ...
    // Başvuran bağışçıya otomatik olarak protokol ve doğrulama kodu mesajı gönder
    const welcomeMsg = {
      id: Date.now() + 1,
...
const AdminDashboard = ({ user, usersList, setUsersList }) => {
  ...
  const handleAddDonation = (e) => {
    ...
    // Update last donation date in usersList
    ...
    // If the currently logged in user matches this T.C., update their session too
    ...
    // Stok artır ve hemen kaydet using finalBlood
    setStockData(prev => {
